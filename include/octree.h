...
public:
	...
	void build();
	void test() const;
	std::tuple< vector<array<double, 3>>, vector<array<int, 1>> > query(array<double, 3> & source, array<double, 3> & dir) const;
...
std::tuple< vector<array<double, 3>>, vector<array<int, 1>> > SDF::query(array<double, 3> & source, array<double, 3> & dir) const {
	/*
	...
	*/
	...
	if (this->is_leaf()) {
		...
		tri_idx_array[0] = (this->indices).at(0);
		bool is_hit = RayTriangle(source, dir, tri, intersection);
		//std::cout << "face index:" << tri_idx << " is_hit:" << is_hit << " intersect:" << intersection.at(0) << " "<< intersection.at(1) << " " << intersection.at(2) << endl;
		if (is_hit == true) {
			vector<array<double, 3>> out = { intersection };
			vector<array<int, 1>> out_tri_idx = {tri_idx_array};
			return std::make_tuple(out, out_tri_idx);
			//return out;
		}
		else {
			vector<array<double, 3>> empty;
			vector<array<int, 1>> out_tri_idx;
			
			return std::make_tuple(empty, out_tri_idx);
			//return empty;
		...
		vector<array<int, 1>> out_face_idx;
		for (size_t i = 0; i < 8; ++i) {
			if (this->children[i] != nullptr) { 
				if (true == RayBox(source, dir, 0, DBL_MAX, this->children[i]->box)) { //leap of faith
					...
					vector<array<int, 1>>  face_idx;
					tie(temp, face_idx) = this->children[i]->query(source, dir);
					out.insert(end(out), begin(temp), end(temp));
					out_face_idx.insert(end(out_face_idx), begin(face_idx), end(face_idx));
				}
			}
		}
		return std::make_tuple(out, out_face_idx); //may be empty
	}
};
...
vector<double> SDF::query(vector<array<double, 3>> & inv_normals) const {
	...
	*/
	...
	for (size_t i = 0; i < inv_normals.size(); ++i) {
		vector<array<double, 3>> intersect;
		vector<array<int, 1>> tmp;
		tie(intersect,  tmp) = this->query((this->V).at(i), inv_normals.at(i));
...
void SDF::test() const {
	...
	std::set<int> idxset_orig(idx_o.begin(), idx_o.end());
